...
def main(**args):
	...
	# Create model
	model = FastDVDnet(num_input_frames = args['temp_patch_size'] )
	...
	for epoch in range(start_epoch, args['epochs']):
		...
		for i, data in enumerate(loader_train, 0):
			...
			# print('data:', data[0]['data'].permute(0,1,4,2,3).shape)
			...
			# noise = torch.zeros_like(img_train, dtype=torch.float32)
		...
		if epoch%5 == 0:
			torch.save(model,f'models/fastdvdnet_{epoch}.pth')
	# Print elapsed time
...
if __name__ == "__main__":
	...
	parser.add_argument("--temp_patch_size", "--tp", type=int, default=3, help="Temporal patch size")
